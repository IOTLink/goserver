...
func (d *dispatcher) dispatch() {
	for {
		select {
		case client := <-d.jobQueue:
			worker := <-d.workerPool
			worker.jobChannel <- client
		case <-d.stop:
...
func newDispatcher(workerPool chan *worker, jobQueue chan net.Conn) *dispatcher {
	d := &dispatcher{
...
func NewPool(numWorkers int, jobQueueLen int) *Pool {
	jobQueue := make(chan net.Conn, jobQueueLen)
...
func (p *Pool) Release() {
	...
	<-p.dispatcher.stop
	fmt.Printf("destory pool success!\n")
}
